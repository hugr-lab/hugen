...
func (s *Session) maybeInjectSoftWarning(runCtx context.Context) {
	...
	text := softWarningText(role, task, st.iter)
...
func softWarningText(role, task string, turns int) string {
	if role == "root" {
	...
	return fmt.Sprintf("You've used %d turns%s. The loop may be productive — but consider stopping to think: is the current approach still right? You can return what you have, give up cleanly explaining what blocked you, or change tack. Sub-sub-agents are not available at this depth.", turns, taskClause)
}
