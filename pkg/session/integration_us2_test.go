...
func TestUS2_PlanSurvivesRestart(t *testing.T) {
	...
	if _, err := callPlanSet(us1WithSession(parent), mgr1, setArgs); err != nil {
	...
	for _, txt := range []string{"checked headers", "instrumented handler"} {
		...
		if _, err := callPlanComment(us1WithSession(parent), mgr1, args); err != nil {
	...
	out, err := callPlanShow(us1WithSession(resumed), mgr2, json.RawMessage(`{}`))
	if err != nil {
...
func TestUS2_PlanEndToEnd(t *testing.T) {
	...
	if _, err := callSpawnSubagent(us1WithSession(parent), mgr, spawnArgs); err != nil {
