...
func (s *stubSource) OwnsState(state string) bool               { return StateOwnedBy(s.name, state) }
...
func TestStateOwnedBy(t *testing.T) {
	assert.True(t, StateOwnedBy("hugr", "hugr.abc123"))
	assert.False(t, StateOwnedBy("hugr", "hugr"))     // no dot suffix
	assert.False(t, StateOwnedBy("hugr", "foo.abc"))  // wrong source
	assert.False(t, StateOwnedBy("hugr", ""))
	assert.Equal(t, "hugr.nonce-1", EncodeState("hugr", "nonce-1"))
}

func TestSourceRegistry_AddAndAlias(t *testing.T) {
	...
	require.True(t, ok)
	assert.Same(t, Source(a), got, "alias should resolve to target")
...
func TestSourceRegistry_DispatchByState(t *testing.T) {
	reg := NewSourceRegistry(nil)
	...
	req := httptest.NewRequest(http.MethodGet,
		"/auth/callback?state="+EncodeState("weather", "nonce")+"&code=xyz", nil)
	...
	req = httptest.NewRequest(http.MethodGet,
		"/auth/callback?state="+EncodeState("hugr", "other")+"&code=xyz", nil)
	w = httptest.NewRecorder()
	mux.ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
...
func TestSourceRegistry_PromptLogins(t *testing.T) {
	reg := NewSourceRegistry(nil)
	called := 0
