...
**IMPORTANT**: There is NO `neq`/`ne`/`not_in` operator. Use `_not` to negate: `{ _not: { field: { eq: "value" } } }`
...
filter: {
  _and: [
    {status: {eq: "active"}}
    {amount: {gt: 1000}}
    # "not equal" — use _not wrapping eq
    {_not: {category: {eq: "internal"}}}
...
| Boolean | eq, is_null |
| DateTime, Timestamp, Date, Time | eq, gt, gte, lt, lte, in, is_null |
